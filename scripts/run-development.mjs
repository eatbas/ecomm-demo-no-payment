...
// The API process needs the JazzCash secrets (see server/config.ts); Vite's
// own .env loading stays disabled (vite.config.ts) so this is the one place
// repository-root `.env` is read, and only to seed this script's own
// process.env before spawning children — never bundled into the browser build.
try {
  ...
  console.warn(
    "No .env file found. Copy .env.example to .env and fill in the JazzCash " +
      "secrets before the API process will start.",
  );
}
