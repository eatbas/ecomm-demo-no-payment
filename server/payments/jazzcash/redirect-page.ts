...
/**
 * A same-origin, server-rendered HTML page that auto-submits the signed
 * card-redirection form to JazzCash's hosted checkout. Values are hashed raw
 * (see buildCardRedirectionFields) and only HTML-escaped here for output, per
 * card-page-redirection gotcha #12. `nonce` must match the `script-src`
 * nonce on this response's own Content-Security-Policy header — the route
 * handler is responsible for generating and sending both together.
 */
