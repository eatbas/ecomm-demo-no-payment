 ...
 * payment_status in lock-step with them, inside the same SQLite connection
 * OrderRepository uses (order status and payment status are one lifecycle by
 * design — see shared/orders.ts).
 */
export class PaymentRepository {
  ...
  initiate(params: InitiatePaymentParams): PaymentRecord {
    ...
    try {
      this.#database
        ...
        .run(id, params.orderId, params.ppTxnRefNo, params.amountPaisa, params.initiatedAt);
  ...
  findByTxnRefNo(ppTxnRefNo: string): PaymentRecord | undefined {
    const row = this.#database
      .prepare(`SELECT ${SELECT_PAYMENT_COLUMNS} FROM payments WHERE pp_txn_ref_no = ?`)
      .get(ppTxnRefNo);
  ...
  /**
   * Idempotent on `pp_TxnRefNo`: JazzCash may legitimately redeliver the same
   * IPN up to three times (ipn-implementation gotcha #9-10), and a payment
   * already marked `paid` is never regressed by a later ambiguous/failed
   * event — a re-delivery after settlement must not unpay a settled order.
   */
  resolve(params: ResolvePaymentParams): PaymentRecord {
    this.#database.exec("BEGIN IMMEDIATE");
    try {
      const existingRow = this.#database
        .prepare("SELECT order_id AS orderId, status FROM payments WHERE pp_txn_ref_no = ?")
